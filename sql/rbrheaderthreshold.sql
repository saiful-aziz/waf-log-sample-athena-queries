WITH t1 AS (
  ...
  FROM waf_logs_partition_projection CROSS JOIN UNNEST (httprequest.headers) AS t(header)
  WHERE   
    log_time >= date_format(current_date - interval '7' day, '%Y/%m/%d') -- Default query is for last  days. Always keep a date filter to reduce the # of records to be looked at 
    -- AND timestamp >=  cast( to_unixtime(from_iso8601_timestamp('2025-07-04T12:00:00'))  as bigint)*1000 -- start timestamp. Change these values to ensure that it aligns with the date filter above
    -- AND timestamp <= cast( to_unixtime(from_iso8601_timestamp('2025-07-11T16:00:00'))  as bigint)*1000 -- end timestamp . Change these values to ensure that it aligns with the date filter above
